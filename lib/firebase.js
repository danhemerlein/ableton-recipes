...
export async function getDefaultPostsList() {
  const postsQuery = noFiltersDecending();
...
export async function filterSortPosts(tagsGenresPlugins, direction) {
  let query;
  ...
  if (tagsGenresPlugins.length) {
    // decesnding
    if (direction.length) {
      query = withFiltersDecending();
      // ascending
    } else {
      query = withFiltersAscending();
    }
  } else {
    // no filters descending
    if (direction.length) {
      query = noFiltersDecending();
      // no filters ascending
    } else {
      query = noFiltersAscending();
    }
  }
